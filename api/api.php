...
function save_item_image($data,$prefix){ if(!$data) return ''; if(!preg_match('/^data:image\/(png|jpeg|jpg|gif|webp);base64,/', $data, $m)) fail('Invalid image format'); $ext=$m[1]==='jpeg'?'jpg':$m[1]; $raw=base64_decode(substr($data, strpos($data, ',')+1)); if($raw===false) fail('Could not read image'); if(strlen($raw)>2*1024*1024) fail('Image must be smaller than 2 MB'); $is_vercel = getenv('VERCEL') || getenv('NOW_REGION'); $dir = $is_vercel ? '/tmp/uploads' : dirname(__DIR__).'/uploads'; if(!is_dir($dir)) mkdir($dir,0777,true); $name=$prefix.'_'.time().'_'.bin2hex(random_bytes(4)).'.'.$ext; file_put_contents($dir.'/'.$name,$raw); return 'uploads/'.$name; }
try {
ensure_user_table();
